...
def validate_multiprocessing(value):
    try:
        ...
        if multiprocessing_value < 0:
            ...
            logging.\
                warning(f"Warning: Multiprocessing value is greater than the number of available CPUs. Setting it to {os.cpu_count()}.")
            return os.cpu_count()
        else:
            return multiprocessing_value
    except ValueError as e:
        logging.error(f"Multiprocessing value cannot be {value}.")
        sys.exit(1)
...
def validate_files_count(value):
    try:
        ...
        if files_count_value < 0:
            logging.error(f"Files count value cannot be negative.")
            sys.exit(1)
        return files_count_value
    except ValueError as e:
        logging.error(f"Files count value cannot be {value}.")
        sys.exit(1)
...
def generate_file_prefix(prefix_choice):
    if prefix_choice == "count":
        ...
        logging.error(f"Invalid choice for file_prefix: {prefix_choice}")
        sys.exit(1)
...
def parse_command_line_args():
    ...
    defaults = {
        ...
        "files_count": validate_files_count(config.get("DEFAULT", "files_count", fallback="0")),
        "file_name": config.get("DEFAULT", "file_name", fallback="filename.txt"),
        "file_prefix": config.get("DEFAULT", "file_prefix", fallback=""),
        ...
        "multiprocessing": validate_multiprocessing(config.get("DEFAULT", "multiprocessing", fallback="1")),
    }
    ...
    parser.add_argument("--file_prefix", choices=["count", "random"], default=defaults["file_prefix"],
